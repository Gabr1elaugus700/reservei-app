...
import { Menu, Bell, Plus, Trash2, Save, Calendar, Settings, Loader2 } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <Card className="p-6">
                ...
                <div key={day.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-500">{day.shortName}</div>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Label htmlFor={`limit-${day.id}`} className="text-sm text-gray-600">
                      Limite:
                    </Label>
                    <Input
                      id={`limit-${day.id}`}
                      ...
                      min="0"
                      value={day.limit}
                      onChange={(e) => updateWeeklyLimit(day.id, parseInt(e.target.value) || 0)}
            ...
            <div className="mt-6 p-4 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-800">
                <strong>Dica:</strong> Os limites padrão serão aplicados automaticamente para todos os dias. 
                Use as configurações de datas específicas abaixo para exceções.
              </p>
            </div>
          </Card>
          ...
          <Card className="p-6">
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-lg">
              <h3 className="text-md font-medium text-gray-900 mb-4">Adicionar Nova Data</h3>
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewSpecialDate(prev => ({ ...prev, limit: e.target.value }))}
                    className="mt-1"
                  />
                </div>
                <div>
            ...
            <div className="space-y-3">
              <h3 className="text-md font-medium text-gray-900">Datas Configuradas</h3>
                  ...
                  <div key={specialDate.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex-1">
                      ...
                      {specialDate.description && (
                        <div className="text-sm text-gray-500">{specialDate.description}</div>
                      )}
                      <div className="text-sm">
                        <span className={`font-medium ${specialDate.limit === 0 ? 'text-red-600' : 'text-green-600'}`}>
                          {specialDate.limit === 0 ? 'Fechado' : `Limite: ${specialDate.limit}`}
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Input
                        ...
                        onChange={(e) => updateSpecialDateLocal(specialDate.id, 'limit', parseInt(e.target.value) || 0)}
                        className="w-20 text-center"
                      />
                      <Button
                        variant="ghost"
            ...
            {specialDates.length > 0 && (
              <div className="mt-6 p-4 bg-green-50 rounded-lg">
                <p className="text-sm text-green-800">
                  <strong>Total:</strong> {specialDates.length} data(s) especial(is) configurada(s). 
                  Essas configurações sobrescreverão os limites padrão da semana.
                </p>
              </div>
            )}
          </Card>
        ...
        <Card className="mt-8 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Resumo das Configurações</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-yellow-50 p-4 rounded-lg">
              <div className="text-2xl font-bold text-yellow-600">
                {Math.round(weeklyLimits.reduce((sum, day) => sum + (day.enabled ? day.limit : 0), 0) / weeklyLimits.filter(d => d.enabled).length)}
              </div>
              <div className="text-sm text-yellow-800">Limite médio semanal</div>
            </div>
